...
public class Lexer {
	...
	private boolean nextIsVariable() {
		...
		{
			//if(tokenString.equals(list.get(i).getVariable()))
			//arr[i] = list.get(i).getVariable().toCharArray();
			System.out.println()	
			}*/
		for(int i=0; i<list.size(); i++)
		{
			if(tokenString.equals(list.get(i).getVariable()))
			//if(textLine.charAt(ndx) == arr[i][0])
				{
				System.out.println("match found");
				tokenString = list.get(i).getValue();
				boolean f = nextIsNumeric();
				//tokenCode = 00;
